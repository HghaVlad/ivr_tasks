...
from tasks.task_checker.task_1 import get_result
app = Flask(__name__)
...
def task_checker():
    if request.method == "GET":
        ...
        if task_id is None:
            ...
            return render_template("fail_form.html", error="Пожалуйста введите ссылку, например: http://example.com")
        elif "http://" not in site_url and "https://" not in site_url:
            return render_template("fail_form.html", error="Пожалуйста введите ссылку, например: http://example.com")
        elif int(task_id) == 1:
            if site_url[-1] != "/":
                site_url = site_url + "/"
